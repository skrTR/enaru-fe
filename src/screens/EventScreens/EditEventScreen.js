...
const EditEventScreen = (props) => {
  ...
  const [selectedDate, setSelectedDate] = useState(data.when ? data.when : "");
  ...
  const [where, setWhere] = useState(data.where ? data.where : "");
  const [contact, setContact] = useState(data.contact ? data.contact : "");
  const [contact2, setContact2] = useState(data.contact2 ? data.contact2 : "");
  ...
  const sendData = () => {
    axios
      .put(`${api}/api/v1/events/${data._id}`, {
        title: title,
